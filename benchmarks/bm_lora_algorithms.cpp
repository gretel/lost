...
const std::vector<uint8_t> PAYLOAD = {'H', 'e', 'l', 'l', 'o', ' ', 'M', 'e', 's', 'h', 'C', 'o', 'r', 'e'};
...
const boost::ut::suite rx_benchmarks = [] {
    ...
    // --- FFT: the single most expensive per-symbol operation ---
    {
        ...
        ::benchmark::benchmark<REPS>(std::string_view("FFT (dechirp+argmax) N=256"), N) = [&fft, &dechirped] {
    ...
    // --- dechirp_argmax: combined dechirp+FFT+argmax (production hot path) ---
    {
        ...
        constexpr int REPS                                                        = 10'000;
        ::benchmark::benchmark<REPS>(std::string_view("dechirp_argmax N=256"), N) = [&fft, &chirp, &downchirp, &scratch] { g_sink = gr::lora::dechirp_argmax(chirp.data(), downchirp.data(), scratch.data(), N, fft); };
    }
    ...
    // --- Dechirp: element-wise complex multiply ---
    {
        ...
        ::benchmark::benchmark<REPS>(std::string_view("dechirp (complex mul) N=256"), N) = [&out, &chirp, &downchirp] {
            for (uint32_t i = 0; i < N; i++) {
    ...
    // --- Gray mapping: trivial per-symbol ---
    {
        ...
        uint16_t      sym                                                        = 142;
        ::benchmark::benchmark<REPS>(std::string_view("gray_map (x^(x>>1))"), 1) = [&sym] {
    ...
    // --- Deinterleave: one block (header block: 8 symbols, sf_app=6) ---
    {
        ...
        auto with_header = gr::lora::insert_header(whitened, static_cast<uint8_t>(PAYLOAD.size()), CR, true);
        ...
        while (payload_syms.size() < cw_len) {
            payload_syms.push_back(0);
        }

        constexpr int REPS                                                                = 10'000;
        ::benchmark::benchmark<REPS>(std::string_view("deinterleave (header, 8 sym)"), 8) = [&header_syms] {
            auto cw = gr::lora::deinterleave_block(header_syms, SF, 8, SF - 2);
            g_sink  = cw.size();
        };

        ::benchmark::benchmark<REPS>(std::string_view("deinterleave (payload, 8 sym)"), cw_len) = [&payload_syms, cw_len] {
    ...
    // --- Hamming decode (hard): per codeword ---
    {
        ...
        constexpr int REPS                                                              = 100'000;
        ::benchmark::benchmark<REPS>(std::string_view("hamming_decode_hard (cr=4)"), 1) = [cw] {
    ...
    // --- Header checksum ---
    {
        ...
        ::benchmark::benchmark<REPS>(std::string_view("header_checksum"), 1) = [] {
            auto ck = gr::lora::header_checksum(static_cast<uint8_t>(PAYLOAD.size()), CR, true);
            g_sink  = ck;
        };
    }
    ...
    // --- Build upchirp: chirp generation ---
    {
        ...
        constexpr int REPS                                                       = 10'000;
        ::benchmark::benchmark<REPS>(std::string_view("build_upchirp N=256"), N) = [&chirp] {
            gr::lora::build_upchirp(chirp.data(), 42, SF, 1);
    ...
    // --- Build ref chirps: upchirp + downchirp pair ---
    {
        ...
        constexpr int REPS                                                          = 10'000;
        ::benchmark::benchmark<REPS>(std::string_view("build_ref_chirps N=256"), N) = [&up, &down] {
    ...
    // --- Interleave: one frame ---
    {
        ...
        ::benchmark::benchmark<REPS>(std::string_view("interleave_frame (37 cw)"), encoded.size()) = [&encoded] {
...
const boost::ut::suite tx_benchmarks = [] {
    ...
    // --- Gray demap: per-symbol TX path ---
    {
        ...
        constexpr int REPS                                                                        = 5'000;
        ::benchmark::benchmark<REPS>(std::string_view("gray_demap (20 sym)"), interleaved.size()) = [&interleaved] {
    ...
    // --- Modulate frame: chirp IQ generation from symbols ---
    {
        ...
        ::benchmark::benchmark<REPS>(std::string_view("modulate_frame (14B, os=1)"), gray_mapped.size()) = [&gray_mapped] {
            auto iq = gr::lora::modulate_frame(gray_mapped, SF, OS_FACTOR, SYNC_WORD, PREAMBLE_LEN);
            g_sink  = iq.size();
        };
    }
...

    {
        ...
        ::benchmark::benchmark<REPS>(std::string_view("generate_frame_iq (14B, os=1)"), PAYLOAD.size()) = [] {
            auto iq = gr::lora::generate_frame_iq(PAYLOAD, SF, CR, OS_FACTOR, SYNC_WORD, PREAMBLE_LEN, true, 0);
            g_sink  = iq.size();
        };
    }
...
const boost::ut::suite misc_benchmarks = [] {
    ...
    // --- Hamming soft decode (ML-LUT): per codeword ---
    {
        ...
        constexpr int REPS                                                              = 100'000;
        ::benchmark::benchmark<REPS>(std::string_view("hamming_decode_soft (cr=4)"), 1) = [&llr] {
...
const boost::ut::suite performance_assertions = [] {
    ...
    "Hamming decode must complete under 1 us"_test = [] {
        ...
        double best_ns = min_of_batches_ns([&] { g_sink = gr::lora::hamming_decode_hard(cw, CR); }, 10'000, 20);
    ...
    "CRC-16 (14 bytes) must complete under 1 us"_test = [] {
        ...
        double best_ns = min_of_batches_ns([&] { g_sink = gr::lora::crc16(PAYLOAD); }, 5'000, 20);
        double best_us = best_ns / 1000.0;
    ...
    "dechirp_argmax must complete under 10 us"_test = [] {
        ...
        double best_ns = min_of_batches_ns([&] { g_sink = gr::lora::dechirp_argmax(chirp.data(), downchirp.data(), scratch.data(), N, fft); }, 500, 20);
        double best_us = best_ns / 1000.0;
