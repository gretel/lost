...
struct AlgorithmRxDecoder {
    ...
    [[nodiscard]] uint16_t demodSymbol(const std::complex<float>* samples) {
        ...
        return static_cast<uint16_t>(gr::lora::mod(static_cast<int64_t>(max_idx) - 1, static_cast<int64_t>(N)));
    }

    [[nodiscard]] static uint16_t grayMap(uint16_t s) { return static_cast<uint16_t>(s ^ (s >> 1)); }

    /// Decode a complete frame from payload IQ symbols (after preamble/SFD).
    /// Returns decoded payload bytes.
    [[nodiscard]] std::vector<uint8_t> decodePayloadSymbols(const std::complex<float>* payload_iq, std::size_t n_symbols) {
        ...
        {
            std::vector<uint16_t> hdr_syms(raw_symbols.begin(), raw_symbols.begin() + 8);
        ...
        if (all_nibbles.size() < 5) {
            return {};
        }
        auto hdr = gr::lora::parse_explicit_header(all_nibbles[0], all_nibbles[1], all_nibbles[2], all_nibbles[3], all_nibbles[4]);
        if (!hdr.checksum_valid || hdr.payload_len == 0) {
            return {};
        }
...
std::size_t countPayloadSymbols(std::size_t payload_len, uint8_t sf, uint8_t cr, bool has_crc) {
    ...
    auto n_payload_syms = static_cast<std::size_t>(std::ceil(static_cast<double>(2 * static_cast<int>(payload_len) - sf + 2 + 5 + (has_crc ? 4 : 0)) / sf_app)) * (4 + cr);
...
const boost::ut::suite decode_benchmarks = [] {
    ...
    auto iq = gr::lora::generate_frame_iq(PAYLOAD, SF, CR, 1, SYNC_WORD, PREAMBLE_LEN, true, 0);
    ...
    std::size_t preamble_samples = static_cast<std::size_t>((PREAMBLE_LEN + 2) * N + N * 2 + N / 4);
    ...
    // --- Benchmark: full RX decode (per-frame) ---
    {
        ...
        ::benchmark::benchmark<REPS>(std::string_view("full RX decode (14B frame)"), 1) = [&decoder, payload_iq, n_payload_symbols] {
    ...
    // --- Benchmark: full TX+RX loopback ---
    {
        ...
        ::benchmark::benchmark<REPS>(std::string_view("TX+RX loopback (14B)"), 1) = [&decoder] {
            auto frame_iq = gr::lora::generate_frame_iq(PAYLOAD, SF, CR, 1, SYNC_WORD, PREAMBLE_LEN, true, 0);

            // RX (skip preamble)
            std::size_t pre_samps = static_cast<std::size_t>((PREAMBLE_LEN + 2) * N + N * 2 + N / 4);
            std::size_t n_syms    = countPayloadSymbols(PAYLOAD.size(), SF, CR, true);

            auto result = decoder.decodePayloadSymbols(frame_iq.data() + pre_samps, n_syms);
            g_sink      = result.size();
        };
    }
