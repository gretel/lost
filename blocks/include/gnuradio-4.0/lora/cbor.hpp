...
inline void encode_head(std::vector<uint8_t>& buf, uint8_t major, uint64_t val) {
...
inline void encode_uint(std::vector<uint8_t>& buf, uint64_t val) { encode_head(buf, 0, val); }
...
inline void encode_bytes(std::vector<uint8_t>& buf, const uint8_t* data, std::size_t len) {
...
inline void encode_map_begin(std::vector<uint8_t>& buf, uint64_t n_pairs) { encode_head(buf, 5, n_pairs); }

inline void encode_array_begin(std::vector<uint8_t>& buf, uint64_t n_items) { encode_head(buf, 4, n_items); }

// Convenience: encode a key-value pair where key is text
inline void kv_uint(std::vector<uint8_t>& buf, const std::string& key, uint64_t val) {
    encode_text(buf, key);
    encode_uint(buf, val);
}

inline void kv_sint(std::vector<uint8_t>& buf, const std::string& key, int64_t val) {
    encode_text(buf, key);
    encode_sint(buf, val);
}

inline void kv_text(std::vector<uint8_t>& buf, const std::string& key, const std::string& val) {
    encode_text(buf, key);
    encode_text(buf, val);
}

inline void kv_bool(std::vector<uint8_t>& buf, const std::string& key, bool val) {
    encode_text(buf, key);
    encode_bool(buf, val);
}

inline void kv_bytes(std::vector<uint8_t>& buf, const std::string& key, const uint8_t* data, std::size_t len) {
    encode_text(buf, key);
    encode_bytes(buf, data, len);
}

inline void kv_float64(std::vector<uint8_t>& buf, const std::string& key, double val) {
    encode_text(buf, key);
...
using Value = std::variant<uint64_t, std::string, std::vector<uint8_t>, bool, std::vector<uint64_t>, double>;
...
inline uint64_t read_argument(std::span<const uint8_t> data, std::size_t& pos, uint8_t info) {
    if (info < 24) {
        return info;
    }
    if (info == 24) {
    ...
    if (info == 25) {
        ...
        uint64_t v = static_cast<uint64_t>(data[pos]) << 8 | static_cast<uint64_t>(data[pos + 1]);
        pos += 2;
        return v;
    }
    if (info == 26) {
        ...
        uint64_t v = static_cast<uint64_t>(data[pos]) << 24 | static_cast<uint64_t>(data[pos + 1]) << 16 | static_cast<uint64_t>(data[pos + 2]) << 8 | static_cast<uint64_t>(data[pos + 3]);
        pos += 4;
        return v;
    }
    if (info == 27) {
...
inline Value decode_item(std::span<const uint8_t> data, std::size_t& pos) {
    ...
    case 2: { // byte string
        ...
        if (pos + len > data.size()) {
            throw DecodeError("byte string overflows input");
        }
    ...
    case 3: { // text string
        uint64_t len = read_argument(data, pos, info);
        if (pos + len > data.size()) {
            throw DecodeError("text string overflows input");
        }
    ...
    case 4: { // array — decode as vector<uint64_t> (only uint elements supported)
        ...
        for (uint64_t i = 0; i < count; i++) {
            ...
            if (!v) {
                throw DecodeError("array element is not uint");
            }
            arr.push_back(*v);
        }
        return arr;
    ...
    default: throw DecodeError("unsupported major type: " + std::to_string(major));
    }
}
...
inline Map decode_map(std::span<const uint8_t> data) {
    ...
    if (major != 5) {
        throw DecodeError("expected CBOR map (major type 5)");
    }
    ...
    for (uint64_t i = 0; i < n_pairs; i++) {
        ...
        if (!key_str) {
            throw DecodeError("map key is not a text string");
        }
...
inline std::vector<uint8_t> get_bytes(const Map& m, const std::string& key) {
    auto it = m.find(key);
    if (it == m.end()) {
        throw DecodeError("missing key: " + key);
    }
    auto* v = std::get_if<std::vector<uint8_t>>(&it->second);
    if (!v) {
        throw DecodeError("key '" + key + "' is not bytes");
    }
    return *v;
}

/// Get a uint value, returning `default_val` if the key is absent.
inline uint64_t get_uint_or(const Map& m, const std::string& key, uint64_t default_val) {
    auto it = m.find(key);
    if (it == m.end()) {
        return default_val;
    }
    ...
    if (!v) {
        throw DecodeError("key '" + key + "' is not uint");
    }
    return *v;
}

/// Get a text value, returning `default_val` if the key is absent.
inline std::string get_text_or(const Map& m, const std::string& key, const std::string& default_val) {
    auto it = m.find(key);
    if (it == m.end()) {
        return default_val;
    }
    ...
    if (!v) {
        throw DecodeError("key '" + key + "' is not text");
    }
    return *v;
}

/// Get a bool value, returning `default_val` if the key is absent.
inline bool get_bool_or(const Map& m, const std::string& key, bool default_val) {
    auto it = m.find(key);
    if (it == m.end()) {
        return default_val;
    }
    ...
    if (!v) {
        throw DecodeError("key '" + key + "' is not bool");
    }
    return *v;
}
...
inline double get_float64_or(const Map& m, const std::string& key, double default_val) {
    auto it = m.find(key);
    if (it == m.end()) {
        return default_val;
    }
...
inline std::vector<uint64_t> get_uint_array_or(const Map& m, const std::string& key) {
    auto it = m.find(key);
    if (it == m.end()) {
        return {};
    }
    ...
    if (!v) {
        throw DecodeError("key '" + key + "' is not a uint array");
    }
    return *v;
}
