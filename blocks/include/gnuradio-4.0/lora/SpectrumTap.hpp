...
struct SpectrumTap : gr::Block<SpectrumTap, gr::NoDefaultTagForwarding> {
    ...
    gr::work::Status processBulk(gr::InputSpanLike auto&  inSpan,
        ...
        for (std::size_t offset = 0; offset < n;) {
            ...
            _sampleBuffer.insert(_sampleBuffer.end(),
                                 inData.begin() + static_cast<std::ptrdiff_t>(offset),
                                 inData.begin() + static_cast<std::ptrdiff_t>(offset + chunk));
            offset += chunk;
    ...
    void processWindow() noexcept {
        ...
        auto fftOut = _fft.compute(
            std::span<const std::complex<float>>(_windowed.data(), fft_size));
    ...
    void publishSpectrum(gr::OutputSpanLike auto& spectrumSpan) noexcept {
        ...
        const float norm = 1.f / (static_cast<float>(fft_size) * static_cast<float>(fft_size)
                                  * static_cast<float>(_windowCount));
        for (uint32_t i = 0; i < fft_size; i++) {
