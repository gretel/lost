...
struct DCBlocker {
    ...
    void init(float sampleRate, float cutoffHz = 2000.f) {
        ...
        auto coeffs = gr::filter::iir::designFilter<double>(gr::filter::Type::HIGHPASS, gr::filter::FilterParameters{.order = kOrder, .fHigh = static_cast<double>(cutoffHz), .fs = static_cast<double>(sampleRate)}, gr::filter::iir::Design::BUTTERWORTH);
    ...
    [[nodiscard]] cf32 processOne(cf32 sample) noexcept {
        ...
        return {static_cast<float>(_filterI.processOne(static_cast<double>(sample.real()))), static_cast<float>(_filterQ.processOne(static_cast<double>(sample.imag())))};
    }
    ...
    void processBlock(std::span<const cf32> in, std::span<cf32> out) noexcept {
        ...
        for (std::size_t i = 0; i < in.size(); ++i) {
            out[i] = {static_cast<float>(_filterI.processOne(static_cast<double>(in[i].real()))), static_cast<float>(_filterQ.processOne(static_cast<double>(in[i].imag())))};
        }
    }
