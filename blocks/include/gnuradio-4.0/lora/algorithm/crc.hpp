...
[[nodiscard]] inline std::array<uint8_t, 4> crc_to_nibbles(uint16_t crc) {
    return {
        static_cast<uint8_t>(crc & 0x0F),
        static_cast<uint8_t>((crc >> 4) & 0x0F),
        static_cast<uint8_t>((crc >> 8) & 0x0F),
        static_cast<uint8_t>((crc >> 12) & 0x0F)
    };
}
...
[[nodiscard]] inline HeaderInfo parse_explicit_header(uint8_t n0, uint8_t n1, uint8_t n2,
