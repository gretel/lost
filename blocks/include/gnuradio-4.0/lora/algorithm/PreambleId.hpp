...
inline std::pair<uint32_t, float> dechirp_quality(
        const cf32* samples, const cf32* ref_chirp,
        cf32* scratch, uint32_t N,
    ...
    for (uint32_t j = 0; j < N; j++) {
        ...
        total += mag_sq;
        if (mag_sq > peak) { peak = mag_sq; peak_bin = j; }
    }
...
inline uint16_t reconstruct_sync_word(int netid1, int netid2, uint32_t N) {
    ...
    int sw0 = static_cast<int>(mod(
        static_cast<int64_t>(std::lround(netid1 / 8.0) * 8),
        static_cast<int64_t>(N)));
    int sw1 = static_cast<int>(mod(
        static_cast<int64_t>(std::lround(netid2 / 8.0) * 8),
        static_cast<int64_t>(N)));
...
inline PreambleInfo characterize_preamble(
        const std::complex<float>* samples,
        std::size_t nSamples,
        uint8_t sf,
        uint32_t bandwidth,
        uint8_t os_factor,
        double center_freq = 868e6,
    ...
    for (std::size_t sym = 0; sym < maxSymbols; ++sym) {
        ...
        auto [bin, pmr] = preamble_detail::dechirp_quality(
            in_down.data(), downchirp.data(), scratch.data(), N, fft);
        ...
        if (prev_bin >= 0 &&
            std::abs(mod(
                std::abs(new_bin - prev_bin) + 1,
                static_cast<int64_t>(N)) - 1) <= sflane_detail::kPreambleBinTolerance) {
            consec_count++;
    ...
    for (uint32_t i = 0; i < up_symb_to_use; i++) {
        sflane_detail::complex_multiply(scratch.data(),
            &preamble_raw[cfo_offset + i * N], downchirp.data(), N);
        ...
        for (uint32_t j = 0; j < N; j++) {
            ...
            if (mag_sq > best) { best = mag_sq; best_idx = static_cast<int>(j); }
        }
        if (best > k0_best_mag) { k0_best_mag = best; idx_max = static_cast<uint32_t>(best_idx); }
    }
    ...
    std::vector<cf32> corr_vec(std::max<std::size_t>(
        static_cast<std::size_t>(n_up_req + 4) * N, 2 * N));
    ...
    sflane_detail::complex_multiply(preamble_upchirps.data(),
        &preamble_raw[cfo_offset], corr_vec.data(), up_symb_to_use * N);
    ...
    for (uint32_t i = 0; i < up_symb_to_use; i++) {
        sflane_detail::complex_multiply(scratch.data(),
            &preamble_upchirps[N * i], downchirp.data(), N);
        auto fft_out = fft.compute(std::span<const cf32>(scratch.data(), N));
        auto to_d = [](cf32 c) {
    ...
    float sto_frac = (std::abs(den) > 1e-15)
        ? std::clamp(static_cast<float>(-std::real(num / den)), -0.5f, 0.5f)
        : 0.f;
    ...
    for (uint32_t extra = 0; extra < 4; ++extra) {
        ...
        for (uint32_t n = 0; n < N; n++) {
            float ph = -2.f * static_cast<float>(std::numbers::pi) * cfo_frac
                     / static_cast<float>(N) * static_cast<float>(n);
    ...
    int down_val = static_cast<int>(dechirp_argmax(
        scratch.data(), upchirp.data(), scratch.data(), N, fft));

    // ==== Integer CFO/STO separation (Xhonneux §5.2) ====
    auto sum_cfo = static_cast<int>(mod(
        static_cast<int64_t>(k_hat) + static_cast<int64_t>(down_val),
        static_cast<int64_t>(N)));
    ...
    // SFO estimation
    float sfo = (static_cast<float>(cfo_int) + cfo_frac)
    ...
    int start_off = os_i / 2
                  - sflane_detail::my_roundf(sto_frac * static_cast<float>(os_factor));
    ...
    for (uint32_t n = 0; n < 2 * N; n++) {
        float ph = -2.f * static_cast<float>(std::numbers::pi)
                 * (static_cast<float>(cfo_int) + cfo_frac)
                 / static_cast<float>(N) * static_cast<float>(n);
        net_id_dec[n] *= cf32(std::cos(ph), std::sin(ph));
    }

    int netid1 = static_cast<int>(dechirp_argmax(
        net_id_dec.data(), downchirp.data(), scratch.data(), N, fft));
    int netid2 = static_cast<int>(dechirp_argmax(
        &net_id_dec[N], downchirp.data(), scratch.data(), N, fft));
    ...
    // Mod-8 STO correction (sync-word-independent, gateway_arch §3)
    {
        ...
        if (r1 == r2 && r1 != 0) {
            ...
            netid1 = static_cast<int>(mod(
                static_cast<int64_t>(netid1) - delta, static_cast<int64_t>(N)));
            netid2 = static_cast<int>(mod(
                static_cast<int64_t>(netid2) - delta, static_cast<int64_t>(N)));
        }
    }
    ...
    for (uint32_t i = 0; i < up_symb_to_use; i++) {
        ...
        if (peak_energy <= 0.f) continue;  // skip zero-energy symbols
    ...
    info.snr_db = (snr_count > 0)
        ? snr_sum / static_cast<float>(snr_count)
        : -999.f;
