...
// Shared constants and helper functions for MultiSfDecoder.
// Lives outside MultiSfDecoder.hpp to avoid GR_REGISTER_BLOCK include conflicts.
...
// Detection thresholds (shared with MultiSfDecoder)
...
/// SF7-10 set to 4.0 as a conservative default; SF11-12 relaxed.
...
[[nodiscard]] inline int my_roundf(float x) noexcept { return x > 0 ? static_cast<int>(x + 0.5f) : static_cast<int>(std::ceil(x - 0.5f)); }

inline void complex_multiply(std::complex<float>* out, const std::complex<float>* a, const std::complex<float>* b, uint32_t n) {
