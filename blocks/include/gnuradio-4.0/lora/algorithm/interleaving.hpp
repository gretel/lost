...
[[nodiscard]] inline std::vector<uint32_t> interleave_block(
        const std::vector<uint8_t>& codewords,  // sf_app codewords
    ...
    for (int i = 0; i < sf_app; i++) {
        if (static_cast<std::size_t>(i) < codewords.size()) {
...
[[nodiscard]] inline std::vector<uint32_t> interleave_frame(
    ...
    while (idx < codewords.size()) {
        ...
        for (int i = 0; i < sf_app && idx < codewords.size(); i++) {
            block.push_back(codewords[idx++]);
        }
        // Pad with zeros if needed (incomplete last block)
        while (static_cast<int>(block.size()) < sf_app) {
            block.push_back(0);
        }
...
[[nodiscard]] inline std::vector<uint8_t> deinterleave_block(
        const std::vector<uint16_t>& symbols,  // cw_len symbols
        uint8_t sf, uint8_t cw_len, uint8_t sf_app) {
    // Convert symbols to binary: cw_len rows x sf_app columns
