...
inline void build_upchirp(std::complex<float>* chirp, uint32_t id, uint8_t sf, uint8_t os_factor = 1) {
    ...
    for (int n = 0; n < len; n++) {
        double phase;
        if (n < n_fold) {
            phase = 2.0 * std::numbers::pi * (static_cast<double>(n) * n / (2.0 * N) / os2 + (static_cast<double>(id) / N - 0.5) * n / os_factor);
        } else {
            phase = 2.0 * std::numbers::pi * (static_cast<double>(n) * n / (2.0 * N) / os2 + (static_cast<double>(id) / N - 1.5) * n / os_factor);
        }
        chirp[n] = std::complex<float>(static_cast<float>(std::cos(phase)), static_cast<float>(std::sin(phase)));
    }
}

/// Build reference upchirp (id=0) and its conjugate (downchirp).
/// Each must point to 2^sf * os_factor elements.
inline void build_ref_chirps(std::complex<float>* upchirp, std::complex<float>* downchirp, uint8_t sf, uint8_t os_factor = 1) {
...
[[nodiscard]] inline uint32_t dechirp_argmax(const std::complex<float>* samples, const std::complex<float>* ref_chirp, std::complex<float>* scratch, uint32_t N, gr::algorithm::FFT<std::complex<float>>& fft) {
...
///
/// scratch must point to N writable elements.
[[nodiscard]] inline DechirpResult dechirp_and_quality(const std::complex<float>* samples, const std::complex<float>* ref_chirp, std::complex<float>* scratch, uint32_t N, gr::algorithm::FFT<std::complex<float>>& fft, bool remove_dc = false) {
    if (remove_dc) {
    ...
    uint32_t max_idx   = 0;
    for (uint32_t i = 0; i < N; i++) {
        float mag = std::sqrt(fft_out[i].real() * fft_out[i].real() + fft_out[i].imag() * fft_out[i].imag());
        total_mag += mag;
        if (mag > max_mag) {
            max_mag = mag;
            max_idx = i;
...
[[nodiscard]] inline DechirpSoftResult dechirp_soft(const std::complex<float>* samples, const std::complex<float>* ref_chirp, std::complex<float>* scratch, uint32_t N, gr::algorithm::FFT<std::complex<float>>& fft) {
    ...
    uint32_t max_idx = 0;
    for (uint32_t i = 0; i < N; i++) {
        float ms         = fft_out[i].real() * fft_out[i].real() + fft_out[i].imag() * fft_out[i].imag();
        result.mag_sq[i] = ms;
