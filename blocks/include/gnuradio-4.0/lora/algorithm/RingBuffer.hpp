...
struct RingBuffer {
    ...
    [[nodiscard]] std::pair<std::span<const cf32>, std::span<const cf32>> recentView(std::size_t n) const noexcept {
        n = std::min(n, count);
        if (n == 0 || data.empty()) {
            return {{}, {}};
        }
        ...
        return {std::span<const cf32>(data.data() + start, firstLen), std::span<const cf32>(data.data(), n - firstLen)};
    }

    [[nodiscard]] std::vector<cf32> recent(std::size_t n) const {
        if (count < n || n > data.size()) {
            return {};
        }
