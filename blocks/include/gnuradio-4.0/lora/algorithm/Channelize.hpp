...
inline void freqShift(std::span<std::complex<float>> samples, double shift_hz, double sample_rate) {
    if (samples.empty()) {
        return;
    }
    const double        phaseInc = 2.0 * std::numbers::pi * shift_hz / sample_rate;
    std::complex<float> phasor(static_cast<float>(std::cos(phaseInc)), static_cast<float>(std::sin(phaseInc)));
...
[[nodiscard]] inline std::vector<std::complex<float>> decimate(const std::complex<float>* src, uint32_t nSamples, uint32_t factor) {
    if (factor <= 1) {
...
[[nodiscard]] inline std::vector<std::complex<float>> channelize(const std::complex<float>* src, uint32_t nSamples, double f_channel, double f_center, double sample_rate, double target_bw) {
...
inline void channelizeFir(const std::complex<float>* src, uint32_t nSamples, double f_channel, double f_center, double sample_rate, CascadedDecimator& decimator, std::vector<std::complex<float>>& out) {
    out.clear();
    if (nSamples == 0) {
        return;
    }
    ...
    const double        shift_hz = -(f_channel - f_center);
    const double        phaseInc = 2.0 * std::numbers::pi * shift_hz / sample_rate;
    std::complex<float> step(static_cast<float>(std::cos(phaseInc)), static_cast<float>(std::sin(phaseInc)));
    std::complex<float> rot(1.0f, 0.0f);

    decimator.reset();
    decimator.processWithNcoBatch(std::span<const std::complex<float>>(src, nSamples), out, rot, step);
}
