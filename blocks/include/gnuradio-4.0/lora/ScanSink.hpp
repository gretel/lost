...
struct ScanSink : gr::Block<ScanSink> {
    ...
    gr::work::Status processBulk(gr::InputSpanLike auto& detectSpan, gr::InputSpanLike auto& spectrumSpan) noexcept {
    ...
    void processDetection(const gr::DataSet<float>& ds) noexcept {
        ...
        gr::lora::log_ts("det  ", "scan", "SF%lld BW%.0fk freq=%.3f MHz ratio=%.1f sweep=%lld", static_cast<long long>(getInt("sf")), static_cast<double>(getFloat("bw")) / 1e3, getDouble("freq") / 1e6, static_cast<double>(getFloat("ratio")), static_cast<long long>(getInt("sweep")));
