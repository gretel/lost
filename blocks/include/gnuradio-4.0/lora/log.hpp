...
namespace detail {
    ...
    [[nodiscard]] inline int level_from_str(const char* lvl) {
        if (lvl[0] == 'd' || lvl[0] == 'D') return static_cast<int>(LogLevel::Debug);
        if (lvl[0] == 'i' || lvl[0] == 'I') return static_cast<int>(LogLevel::Info);
        if (lvl[0] == 'w' || lvl[0] == 'W') return static_cast<int>(LogLevel::Warning);
        if (lvl[0] == 'e' || lvl[0] == 'E') return static_cast<int>(LogLevel::Error);
...
inline void set_log_level(std::string_view level_str) {
    if (level_str.empty()) return;
    detail::g_log_level.store(detail::level_from_str(level_str.data()),
