...
def main() -> None:
    ...
    try:
        for msg in read_cbor_seq(sys.stdin.buffer):
            ...
            # Frame identity now lives under `carrier` (sync_word/sf/bw/cr);
            # DSP state under `phy`.  Filter by carrier.sync_word.
            ...
            if sync_word != LORAWAN_SYNC_WORD:
                if args.all:
                    ...
                    tag_str = f" [{decode_label}]" if decode_label else ""
                    print(
                        f"[{msg.get('ts', '')}] {dir_tag}#{msg.get('seq', 0)}  "
                        f"{len(payload)} bytes  {crc}  [sync=0x{sync_word:02X}]{tag_str}"
                    )
