...
# All matrices lock to 869.618 MHz (the SDR's tuned frequency).
# The SDR does NOT retune per-point — only the Heltec TX does.
...
MATRICES: dict[str, list[ConfigPoint]] = {
    ...
    "bridge_full": [
        ConfigPoint(sf=7, bw=62500, freq_mhz=FREQ_MHZ),  # 652ms
        ConfigPoint(sf=8, bw=62500, freq_mhz=FREQ_MHZ),  # 1140ms
        ConfigPoint(sf=9, bw=125000, freq_mhz=FREQ_MHZ),  # 1042ms
        ConfigPoint(sf=10, bw=250000, freq_mhz=FREQ_MHZ),  # 1042ms
    ],
    # Official MeshCore regional presets (SF/BW/CR vary, freq locked).
    "meshcore_presets": [
        ConfigPoint(
            sf=8, bw=62500, freq_mhz=FREQ_MHZ, cr=8, tx_power=10
        ),  # EU/UK Narrow
        ConfigPoint(
            sf=11, bw=250000, freq_mhz=FREQ_MHZ, cr=5, tx_power=10
        ),  # EU/UK Long Range
        ConfigPoint(
            sf=10, bw=250000, freq_mhz=FREQ_MHZ, cr=5, tx_power=10
        ),  # EU/UK Medium Range
        ConfigPoint(sf=7, bw=62500, freq_mhz=FREQ_MHZ, cr=5, tx_power=10),  # CZ Narrow
    ],
    # Comprehensive baseline: SF7-12 x BW62.5k/125k/250k. tx_power=10.
    "baseline": [
        # BW62.5k: all SFs
...
class PointResult:
    ...
    det_count: int = 0  # cluster count (burst-deduped), not raw hit count
...
def run_point(
    ...
    if mode == "decode":
        # SF12/BW62.5k airtime is ~4.5s; use airtime + 3s margin so the
    ...
    # TX ADVERT(s) — send multiple to increase detection probability
    # at marginal SNR. Scan and decode both use 3 repeats with gaps.
    ok_count = 0
    tx_repeats = 3
    ...
    _info(f"  TX {ok_count}/{tx_repeats} ok")
...
def _collect_scan(
    ...
    Clusters per-sweep detections into burst-level detections: one ADVERT
    burst (~500 ms at SF8, ~4.5 s at SF12) crosses many sweep windows and
    would otherwise be counted as N independent detections. Two detections
    on the same frequency bin within ``kMaxGap`` consecutive sweeps are
    merged; ``det_count`` is the cluster count, ``raw_det_count`` is the
    pre-clustering per-sweep hit count.
    """
    ...
    sweep_durs: list[int] = []
    hits: list[tuple[int, dict[str, Any]]] = []  # (sweep_idx, detection)
    sweep_idx = 0
    for ev in events:
        t = ev.get("type", "")
        if t == "scan_sweep_end":
    ...
    # Cluster by (freq_bin, sweep_gap). 62.5 kHz grid matches the scan
    ...
    for sweep, d in hits:
        fb = int(round(d.get("freq", 0.0) / kFreqBinHz))
    ...
    # Keep best-ratio entry per cluster as the cluster representative.
...
async def run_bridge_point(
    ...
    # Step 0: Reconfigure TX side only.
    # lora_trx PHY is configured at startup (multi-BW + promiscuous +
    # SF7-12 sweep in the default apps/config.toml), so the matrix
    # point's SF/BW/freq are caught automatically on the RX side — no
    # runtime reconfig needed.  The Heltec companion still needs to be
    # retuned per point for TX.
    #
...
def main() -> None:
    ...
    for name in ("decode", "scan", "tx", "bridge"):
        ...
        p.add_argument(
            "--binary",
            default="",
            help="Override SDR binary path (default: ./build/apps/lora_trx or lora_scan). "
            "Use to select an alternate build for A/B runs.",
        )
        p.add_argument(
            "--host",
            default="127.0.0.1",
            help="SDR host for UDP subscription (default: 127.0.0.1). "
            "Use with --attach for remote SDR (e.g. --host 10.0.23.159).",
        )
    ...
    if mode in ("decode", "tx", "bridge"):
        binary = args.binary or "./build/apps/lora_trx"
        udp_port = TRX_PORT
        event_types = {"lora_frame"}
    else:
        binary = args.binary or "./build/apps/lora_scan"
        udp_port = 5557
    ...
    sock, _, _ = create_udp_subscriber(args.host, udp_port)
...
async def _run_bridge_experiment(
    ...
    *,
    ...
    config_addr = (args.host, TRX_PORT)
