...
class Candidate:
    ...
    sync_word: int  # from carrier.sync_word
    sf: int  # from carrier.sf
    arrived_at: float  # time.monotonic()


# Dedup key: (payload_hash, sync_word).  Carrier identity (sync) +
# content identity (hash).  SF intentionally omitted — within a single
# OTA event all diversity paths see the same SF, and cross-SF
# same-payload transmissions are logically one message (ADR retries).
...
def extract_candidate(msg: dict[str, Any], arrived_at: float) -> Candidate | None:
    ...
    return Candidate(
        ...
        frame_id=frame_id,
        sync_word=int(carrier.get("sync_word", 0)),
        sf=int(carrier.get("sf", 0)),
...
def format_ruling(group: PendingGroup) -> str:
    ...
    parts.append(f"sync=0x{winner.sync_word:02X}")
...
def run(args: argparse.Namespace) -> None:
    ...
    # Aggregation state
    pending: dict[DedupKey, PendingGroup] = {}
    recently_emitted: dict[DedupKey, float] = {}  # dedup cooldown
    tx_echo_hashes: dict[DedupKey, float] = {}    # TX payload → transmit ts
    ...
    def emit_group(group: PendingGroup) -> None:
        ...
        recently_emitted[group.key] = time.monotonic()
        msg = build_aggregated(group)
        data = cbor2.dumps(msg)
        server.broadcast(data, group.sync_word)
        frames_total += 1
        frames_since_log += 1
    ...
    def drain_expired() -> None:
        ...
        stale = [k for k, t in recently_emitted.items() if (now - t) >= DEDUP_COOLDOWN]
        for k in stale:
            del recently_emitted[k]
        # Purge stale TX echo entries
        tx_stale = [k for k, t in tx_echo_hashes.items() if (now - t) >= TX_ECHO_WINDOW]
        for k in tx_stale:
            del tx_echo_hashes[k]
    ...
    def handle_upstream_frame(msg: dict[str, Any]) -> None:
        ...
        # TX frames come through as lora_frame with direction="tx" since the
        # tx_worker commit landed.  Record hash+sync for echo suppression and
        # forward unchanged (consumers interested in TX history read it raw).
        direction = msg.get("direction", "rx")
        if direction == "tx":
            carrier = msg.get("carrier", {})
            phash = msg.get("payload_hash")
            sync = int(carrier.get("sync_word", 0))
            if phash is not None:
                key: DedupKey = (int(phash), sync)
                tx_echo_hashes[key] = time.monotonic()
                log.debug(
                    "TX echo: recorded key=(%016x, 0x%02X)", int(phash), sync
                )
            # Forward TX frame filtered by sync so sync-filtered consumers see it too.
            server.broadcast(cbor2.dumps(msg), sync if sync else None)
            return

        now = time.monotonic()
        cand = extract_candidate(msg, now)
        if cand is None:
            # payload_hash missing — pass through without aggregation
            data = cbor2.dumps(msg)
            server.broadcast(data)
            return
        key = (cand.msg["payload_hash"], cand.sync_word)

        # TX echo suppression: drop RX frames matching recent TX transmissions.
        if key in tx_echo_hashes:
            tx_age = now - tx_echo_hashes[key]
            log.info(
                "TX echo suppressed: key=(%016x, 0x%02X) (%.0fms after TX)",
                key[0], key[1], tx_age * 1000,
            )
            del tx_echo_hashes[key]  # one-shot: suppress once, then clear
            tx_echoes_suppressed += 1
            return

        if key in recently_emitted:
            log.debug(
                "dedup key=(%016x, 0x%02X) (%.0fms late)",
                key[0], key[1],
                (now - recently_emitted[key]) * 1000,
            )
            return
        if key not in pending:
            pending[key] = PendingGroup(key=key, created_at=now)
        group = pending[key]
            ...
            for key, _ in events:
                ...
                if sock is server.sock:
                    ...
                    if tx_msg is not None and up_addrs:
                        # TX echo suppression is now driven by direction="tx"
                        # lora_frame broadcasts from lora_trx itself (captured
                        # in handle_upstream_frame).  No need to hash the
                        # client → server TX request here — we rely on the
                        # upstream actually reporting what it transmitted.
