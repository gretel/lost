...
from conftest import (
    make_identity,
    make_two_identities as _make_two_identities,
    make_state,
    make_contact_record as _make_contact_record,
    FakeUDPSock as _FakeUDPSock,
)
...
class TestEepromEmulation(unittest.TestCase):
    ...
    def test_persist_updates_startup_mirrors(self):
        ...
        When settings are persisted to EEPROM, the _startup_* mirrors must
        also update so that on the next APP_START, apply_startup_config()
        restores the correct persisted values.
        Note: send_scope is NOT persisted — CLI controls scope per-session.
        """
        state = make_state()
