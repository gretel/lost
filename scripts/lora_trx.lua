...
function lora.dissector(tvbuf, pinfo, tree)
    ...
    if msg_type == "lora_frame" then
        pinfo.cols.info = string.format("RX #%s", tostring(result["seq"] or "?"))
        ...
        if type(phy) == "table" then
            ...
            if phy["crc_valid"] ~= nil then phy_tree:add(f_crc_valid, tvbuf(), phy["crc_valid"]) end
            ...
            local crc_str = "?"
            if phy["crc_valid"] == true then crc_str = "OK"
            elseif phy["crc_valid"] == false then crc_str = "FAIL" end

            pinfo.cols.info = string.format(
                "RX #%d %dB SF%d CRC_%s SNR=%.1f",
                result["seq"] or 0,
