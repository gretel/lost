...
Provides:
  ...
  - TOML loader (CBOR config from lora_trx is preferred)
...
def format_frame(
    ...
    *,
    ...
    2. Protocol + decrypted content (MeshCore only, via _parse_summary/_decrypt)
    3. Hex dump (compact, omitted when decrypted content is shown)
    4. Diversity line (aggregated frames from lora_agg only)

    _parse_summary and _decrypt are optional callables injected by lora_mon.py
    for MeshCore protocol parsing and decryption. When None, those blocks are skipped.
    """
    payload = msg.get("payload", b"")
    # Frame identity lives in `carrier` (sync_word/sf/bw/cr); DSP state in `phy`.
...
def config_udp_port(cfg: dict[str, Any]) -> int:
    ...
    Returns the lora_trx port (default 5555 when no section; 5556 when [network]
    section is present). Scripts that want the aggregated consumer stream
    should use config_agg_listen() instead.
    """
    net = cfg.get("network", {})
...
def resolve_udp_address(
    ...
    An empty [aggregator] section (``{}``) still counts as present — the C++
    side fills defaults from missing keys, and Python mirrors that semantic
    with ``is not None`` rather than a truthy check (which would reject
    ``{}``).
    """
    if connect_arg:
...
def wait_for_config(
    ...
    *,
    ...
    ``rx_chains``, and ``raw`` sub-dicts).  Raises ``TimeoutError`` if
...
# Runtime PHY reconfiguration (send_lora_config) was removed.  PHY
# parameters are configuration-only now: edit apps/config.toml and
# restart lora_trx.  See CBOR-SCHEMA.md and apps/config.toml for the
# rationale (multi-chain + promiscuous RX broke the old broadcast-to-all
# semantics of the lora_config message).
