...
struct ClientEntry {
    ...
    [[nodiscard]] bool accepts(uint16_t sw) const { return sync_words.empty() || std::ranges::find(sync_words, sw) != sync_words.end(); }
};
...
struct UdpState {
    ...
    void broadcast(const std::vector<uint8_t>& buf, uint16_t sync_word) { sendToAll(buf, &sync_word); }

    /// Broadcast to ALL clients, ignoring sync_word filters.
    void broadcast_all(const std::vector<uint8_t>& buf) { sendToAll(buf, nullptr); }

private:
    void sendToAll(const std::vector<uint8_t>& buf, const uint16_t* filter_sw) {
        std::lock_guard<std::mutex> lock(mutex);
        std::erase_if(clients, [](const auto& c) { return c.send_failures >= kMaxSendFailures; });
        for (auto& client : clients) {
            if (filter_sw && !client.accepts(*filter_sw)) {
                continue;
            }
            auto len = (client.addr.ss_family == AF_INET6) ? sizeof(struct sockaddr_in6) : sizeof(struct sockaddr_in);
            auto rc  = ::sendto(fd, buf.data(), buf.size(), 0, reinterpret_cast<const struct sockaddr*>(&client.addr), static_cast<socklen_t>(len));
            if (rc < 0) {
...
public:
    static bool sockaddr_equal(const struct sockaddr_storage& a, const struct sockaddr_storage& b) {
        ...
        if (a.ss_family == AF_INET6) {
            ...
            return a6.sin6_port == b6.sin6_port && std::memcmp(&a6.sin6_addr, &b6.sin6_addr, 16) == 0;
        }
    ...
    void subscribe(const struct sockaddr_storage& sender, std::vector<uint16_t> sync_words = {}) {
        std::lock_guard<std::mutex> lock(mutex);
        for (auto& c : clients) {
        ...
        if (entry.sync_words.empty()) {
            gr::lora::log_ts("info ", "udp", "client %s:%u subscribed (all, %zu total)", addr_str, port, clients.size());
        } else {
            std::string filter;
            ...
            gr::lora::log_ts("info ", "udp", "client %s:%u subscribed (sync=%s, %zu total)", addr_str, port, filter.c_str(), clients.size());
        }
    }

    void sendTo(const std::vector<uint8_t>& buf, const struct sockaddr_storage& dest) {
        auto len = (dest.ss_family == AF_INET6) ? sizeof(struct sockaddr_in6) : sizeof(struct sockaddr_in);
        ::sendto(fd, buf.data(), buf.size(), 0, reinterpret_cast<const struct sockaddr*>(&dest), static_cast<socklen_t>(len));
    }
};
...
inline int create_udp_socket(const std::string& listen_addr, uint16_t port) {
    ...
    if (::inet_pton(AF_INET6, listen_addr.c_str(), &bind6.sin6_addr) == 1) {
        ...
        gr::lora::log_ts("error", "udp", "invalid listen address '%s'", listen_addr.c_str());
        return -1;
    }
    ...
    if (use_v6) {
        ...
        if (::bind(fd, reinterpret_cast<struct sockaddr*>(&bind6), sizeof(bind6)) < 0) {
            gr::lora::log_ts("error", "udp", "bind([%s]:%u): %s", listen_addr.c_str(), port, std::strerror(errno));
            ::close(fd);
            return -1;
        }
    } else {
    ...
    // Plumbing detail — the human-facing "telemetry udp://addr:port" line is
    // emitted by the caller after a successful bind.
    gr::lora::log_ts("debug", "udp", "bound %s:%u%s", listen_addr.c_str(), port, use_v6 ? " (IPv6 dual-stack)" : "");
    return fd;
}
