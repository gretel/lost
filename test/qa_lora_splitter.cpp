...
SplitResult runSplitter(uint32_t nOutputs, const std::vector<cf32>& inputSamples, const std::vector<gr::Tag>& inputTags = {}) {
    using namespace gr;
    ...
    auto& src  = graph.emplaceBlock<testing::TagSource<cf32, testing::ProcessFunction::USE_PROCESS_BULK>>({
    ...
    for (uint32_t i = 0; i < nOutputs; i++) {
        auto& sink = graph.emplaceBlock<testing::TagSink<cf32, testing::ProcessFunction::USE_PROCESS_BULK>>({
            {"log_samples", true},
            {"log_tags", true},
        });
        ...
        if (!graph.connect(splitter, portName, sink, "in"s)) {
            throw std::runtime_error(std::format("failed to connect splitter.out[{}] -> sink[{}]", i, i));
        }
    }
...
const boost::ut::suite<"Splitter N=1 passthrough"> n1_tests = [] {
    ...
    "Samples pass through unchanged with N=1"_test = [] {
        ...
        expect(eq(result.outputs[0].size(), 3UZ)) << "Expected 3 samples, got " << result.outputs[0].size();
...
const boost::ut::suite<"Splitter N=2 copy"> n2_tests = [] {
    ...
    "Both outputs receive identical samples"_test = [] {
        ...
        for (std::size_t port = 0; port < 2; port++) {
            expect(eq(result.outputs[port].size(), 3UZ)) << "Port " << port << " expected 3 samples";
    ...
    "Tags forwarded to both outputs"_test = [] {
        ...
        expect(eq(result.outputs.size(), 2UZ));
        for (std::size_t port = 0; port < 2; port++) {
            expect(ge(result.tags[port].size(), 1UZ)) << "Port " << port << " should have at least 1 tag";
            if (!result.tags[port].empty()) {
                bool hasBurstStart = result.tags[port][0].map.contains("burst_start");
                expect(hasBurstStart) << "Port " << port << " should have burst_start tag";
            }
        }
    };
};
...
const boost::ut::suite<"Splitter N=3 copy"> n3_tests = [] {
    ...
    "Three-way split produces identical outputs"_test = [] {
        ...
        for (std::size_t port = 0; port < 3; port++) {
            expect(eq(result.outputs[port].size(), 2UZ)) << "Port " << port << " expected 2 samples";
