...
std::vector<cf32> makeTone(std::size_t nSamples, double freq_hz, double sample_rate, float amplitude = 1.0f) {
    std::vector<cf32> out(nSamples);
    const double      phaseInc = 2.0 * std::numbers::pi * freq_hz / sample_rate;
    double            phase    = 0.0;
    for (auto& s : out) {
        s = cf32(amplitude * static_cast<float>(std::cos(phase)), amplitude * static_cast<float>(std::sin(phase)));
        phase += phaseInc;
    }
    return out;
}
...
const boost::ut::suite<"Channelize"> channelizeTests = [] {
    ...
    "channelize extracts off-center tone"_test = [] {
        ...
        expect(std::abs(sumReal) > static_cast<float>(nb.size()) * 0.5f) << "tone at DC after channelization";
    };
};
...
const boost::ut::suite<"RingBuffer"> ringBufferTests = [] {
    ...
    "recent wraps around correctly"_test = [] {
        ...
        for (int i = 0; i < 5; ++i) {
            expect(out[static_cast<std::size_t>(i)].real() == static_cast<float>(10 + i)) << "sample " << i;
        }
    };
...
const boost::ut::suite<"SpectrumTap"> spectrumTapTests = [] {
    ...
    "passthrough preserves samples"_test = [] {
        ...
        auto& source = graph.emplaceBlock<gr::testing::TagSource<cf32>>({{"n_samples_max", static_cast<gr::Size_t>(totalSamples)}, {"mark_tag", false}});

        auto& tap = graph.emplaceBlock<gr::lora::SpectrumTap>({{"sample_rate", sampleRate}, {"fft_size", fftSize}, {"fft_accumulate", accumulate}});

        auto& sink = graph.emplaceBlock<gr::testing::TagSink<cf32, gr::testing::ProcessFunction::USE_PROCESS_BULK>>({{"log_samples", false}, {"log_tags", false}});
        ...
        expect(eq(sink._nSamplesProduced, static_cast<gr::Size_t>(totalSamples))) << "all samples passed through";
    };
};
...
const boost::ut::suite<"ScanController L1 energy detection"> scanControllerL1Tests = [] {
    ...
    auto makeScanController = [](float sampleRate = 1e6f, float centerFreq = 868e6f, float channelBw = 62500.f, float minRatio = 8.0f, uint32_t fftSize = 1024, uint32_t interval = 1, uint32_t snapshots = 4) {
    ...
    "channel count matches usable bandwidth"_test = [&] {
        ...
        expect(eq(sc._l1._nChannels, 12U)) << "12 channels in 800 kHz usable BW";
    };
    ...
    "channelCenterFreq returns correct frequencies"_test = [&] {
        ...
        for (uint32_t ch = 0; ch < sc._l1._nChannels; ++ch) {
            const double expected = bandStart + (static_cast<double>(ch) + 0.5) * 62500.0;
            expect(std::abs(sc.channelCenterFreq(ch) - expected) < 1.0) << "channel " << ch << " center frequency";
        }
    };
    ...
    "white noise produces no hot channels"_test = [&] {
        ...
        expect(eq(sc._l1._snapshotCount, sc.l1_snapshots)) << "all snapshots accumulated";

        // Find hot channels — white noise should have uniform energy, no outliers
        sc.findHotChannels();
        expect(sc._hotChannels.empty()) << "white noise should produce no hot channels, got " << sc._hotChannels.size();
    };
    ...
    "CW tone produces detection at correct channel"_test = [&] {
        ...
        auto sc = makeScanController(static_cast<float>(sampleRate), 868e6f, static_cast<float>(channelBw), 8.0f, fftSize, 1, snaps);
        ...
        const uint32_t   expectedCh = static_cast<uint32_t>((toneOffset + 0.5 * sampleRate * 0.8) / channelBw);
        ...
        if (!sc._hotChannels.empty()) {
            ...
            expect(found) << "hot channel near expected channel " << expectedCh << ", got channels: " << [&] {
                std::string s;
        ...
        float              toneEnergy = sc._l1._channelEnergy[expectedCh];
        std::vector<float> sorted(sc._l1._channelEnergy.begin(), sc._l1._channelEnergy.begin() + static_cast<std::ptrdiff_t>(sc._l1._nChannels));
    ...
    "two CW tones at different frequencies"_test = [&] {
        ...
        const uint32_t expectedChA = static_cast<uint32_t>((toneA + 0.5 * sampleRate * 0.8) / channelBw);
        const uint32_t expectedChB = static_cast<uint32_t>((toneB + 0.5 * sampleRate * 0.8) / channelBw);
        ...
        expect(sc._hotChannels.size() >= 2U) << "two tones should produce at least 2 hot channels, got " << sc._hotChannels.size();

        // Check both expected channels are found (within ±1 tolerance)
        auto channelFound = [&](uint32_t expected) {
            for (uint32_t ch : sc._hotChannels) {
                if (ch >= expected - 1 && ch <= expected + 1) {
                    return true;
                }
            }
            return false;
        };
        expect(channelFound(expectedChA)) << "tone A channel " << expectedChA << " should be hot";
        expect(channelFound(expectedChB)) << "tone B channel " << expectedChB << " should be hot";
    };
    ...
    "cluster dedup merges adjacent hot bins to single channel"_test = [&] {
        ...
        sc._l1._channelEnergy.assign(sc._l1._nChannels, 1.0f);
        sc._l1._snapshotCount = 4; // non-zero so findHotChannels proceeds

        // Make channels 5, 6, 7 hot (6 is peak)
        sc._l1._channelEnergy[5] = 100.f;
        sc._l1._channelEnergy[6] = 200.f;
        sc._l1._channelEnergy[7] = 150.f;

        sc.findHotChannels();
        // Cluster dedup should merge channels 5-7 into a single detection at channel 6 (peak)
        expect(eq(sc._hotChannels.size(), 1UZ)) << "adjacent hot channels should be merged into 1 cluster";
        if (!sc._hotChannels.empty()) {
            expect(eq(sc._hotChannels[0], 6U)) << "cluster peak should be channel 6";
        }
    };

    "findHotChannels with no snapshots returns empty"_test = [&] {
        auto sc               = makeScanController();
        sc._l1._snapshotCount = 0;
        sc.findHotChannels();
    ...
    "findHotChannels reports on first sweep and tracks persistence"_test = [&] {
        auto sc  = makeScanController();
        auto nCh = sc._l1._nChannels;
        expect(gt(nCh, 0U)) << "should have channels";

        // Sweep 1: one channel hot → reported immediately (kMinHotSweeps = 1)
        sc._l1._channelEnergy.assign(nCh, 1.0f);
        if (nCh > 5) {
            sc._l1._channelEnergy[5] = 100.0f;
        }
        sc._l1._snapshotCount = 1;
        sc.findHotChannels();
        expect(eq(sc._hotChannels.size(), 1UZ)) << "should report on first sweep";
        if (!sc._hotChannels.empty()) {
        ...
        sc._l1._snapshotCount = 1;
        sc.findHotChannels();
        expect(sc._hotChannels.empty()) << "should NOT report after channel goes cold";
        ...
        sc._l1._snapshotCount = 1;
        sc.findHotChannels();
        expect(eq(sc._hotChannels.size(), 1UZ)) << "should report immediately when hot again";
    };
    ...
    "resetAccumulation does not clear persistence counters"_test = [&] {
        ...
        expect(eq(sc._l1._channelHotCount[5], 1U)) << "persistence counter survives resetAccumulation";
        ...
        sc._l1._snapshotCount = 1;
        sc.findHotChannels();
        expect(eq(sc._hotChannels.size(), 1UZ)) << "channel still reported after persistence survives reset";
    };
    ...
    "computeEnergySnapshot requires sufficient ring buffer data"_test = [&] {
        ...
        sc.computeEnergySnapshot();
        expect(eq(sc._l1._snapshotCount, 0U)) << "snapshot should not be counted when ring buffer has insufficient data";
    };

    "resetAccumulation clears energy and counters"_test = [&] {
        auto sc = makeScanController();
        // Inject some state
        sc._l1._channelEnergy[0] = 999.f;
        sc._l1._snapshotCount    = 10;
        sc._callCount            = 42;
        sc._hotChannels.push_back(3);

        sc.resetAccumulation();

        expect(eq(sc._l1._channelEnergy[0], 0.f)) << "energy cleared";
        expect(eq(sc._l1._snapshotCount, 0U)) << "snapshot count cleared";
    ...
    "DC-adjacent hot channels excluded from probe window"_test = [&] {
        ...
        sc._l1._channelEnergy.assign(nCh, 1.0f);
        sc._l1._channelEnergy[centerCh] = 200.f; // DC spur
        sc._l1._channelEnergy[5]        = 150.f; // real signal, dist=20 from center
        sc._l1._snapshotCount           = 1;

        // findHotChannels still reports both (it doesn't filter DC)
        sc.findHotChannels();
        expect(eq(sc._hotChannels.size(), 2UZ)) << "findHotChannels reports both DC and off-center";
        ...
        const uint32_t  usableRadius = static_cast<uint32_t>(sc.sample_rate * kUsableFrac / (2.f * sc.channel_bw));
    ...
    "DC-adjacent hot channels at edge of exclusion zone"_test = [&] {
        ...
        sc._l1._channelEnergy.assign(nCh, 1.0f);
        sc._l1._channelEnergy[36] = 200.f; // dist=11, at DC edge
        sc._l1._channelEnergy[38] = 200.f; // dist=13, outside DC
        sc._l1._snapshotCount     = 1;
        sc.findHotChannels();
    ...
    "state machine transitions ACCUMULATE → findHotChannels on snapshot threshold"_test = [&] {
        ...
        expect(eq(sc._l1._snapshotCount, snaps)) << "snapshot count reached threshold";
        expect(sc._state == gr::lora::ScanController::ScanState::Accumulate) << "still in Accumulate before findHotChannels";
