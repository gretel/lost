...
std::vector<cf32> makeBurst(std::size_t n, float offset = 0.0f) {
    ...
    for (std::size_t i = 0; i < n; i++) {
        burst[i] = cf32{offset + static_cast<float>(i), -(offset + static_cast<float>(i))};
    }
    return burst;
}
...
DrainResult runTxQueue(SetupFn setupFn, PostStartFn postStartFn, std::chrono::milliseconds timeout = 2000ms) {
    using namespace gr;
    ...
    auto& sink0 = graph.emplaceBlock<testing::TagSink<cf32, testing::ProcessFunction::USE_PROCESS_BULK>>({
        {"log_samples", true},
        {"log_tags", false},
    });
    auto& sink1 = graph.emplaceBlock<testing::TagSink<cf32, testing::ProcessFunction::USE_PROCESS_BULK>>({
        {"log_samples", true},
        {"log_tags", false},
    });
    ...
    if (auto ret = sched.exchange(std::move(graph)); !ret) {
        throw std::runtime_error(std::format("scheduler exchange failed: {}", ret.error()));
    }
    ...
    std::thread schedThread([&sched]() { std::ignore = sched.runAndWait(); });
...
DrainResult runTxQueue(SetupFn setupFn, std::chrono::milliseconds timeout = 2000ms) {
    return runTxQueue(
        setupFn, [](gr::lora::TxQueueSource&, auto&) {}, // no post-start action
        timeout);
}
...
const boost::ut::suite<"TxQueueSource empty queue"> empty_tests = [] {
    ...
    "empty queue produces zero samples"_test = [] {
        ...
        expect(eq(result.out0_samples.size(), 0UZ)) << "out0 should have 0 samples, got " << result.out0_samples.size();
        expect(eq(result.out1_samples.size(), 0UZ)) << "out1 should have 0 samples, got " << result.out1_samples.size();
    };
};
...
const boost::ut::suite<"TxQueueSource single burst"> single_burst_tests = [] {
    ...
    "push and drain single burst"_test = [] {
        ...
        expect(eq(result.out0_samples.size(), kBurstLen)) << "out0 expected " << kBurstLen << " samples, got " << result.out0_samples.size();

        if (result.out0_samples.size() == kBurstLen) {
            for (std::size_t i = 0; i < kBurstLen; i++) {
                expect(eq(result.out0_samples[i], burst[i])) << "out0 mismatch at sample " << i;
            }
        }

        // Verify out1 has matching zeros.
        expect(eq(result.out1_samples.size(), kBurstLen)) << "out1 expected " << kBurstLen << " samples, got " << result.out1_samples.size();

        if (result.out1_samples.size() == kBurstLen) {
            bool allZero = std::all_of(result.out1_samples.begin(), result.out1_samples.end(), [](cf32 v) { return v == cf32{}; });
...
const boost::ut::suite<"TxQueueSource multiple bursts"> multi_burst_tests = [] {
    ...
    "multiple bursts drain sequentially"_test = [] {
        ...
        expect(eq(result.out0_samples.size(), totalExpected)) << "out0 expected " << totalExpected << " samples, got " << result.out0_samples.size();

        if (result.out0_samples.size() == totalExpected) {
            // First burst: samples 0..49 should match burst1.
            for (std::size_t i = 0; i < kBurst1Len; i++) {
                expect(eq(result.out0_samples[i], burst1[i])) << "burst1 mismatch at sample " << i;
            }
            // Second burst: samples 50..79 should match burst2.
            for (std::size_t i = 0; i < kBurst2Len; i++) {
                expect(eq(result.out0_samples[kBurst1Len + i], burst2[i])) << "burst2 mismatch at sample " << i;
            }
        }

        // out1 should have matching zeros, same total count.
        expect(eq(result.out1_samples.size(), totalExpected)) << "out1 expected " << totalExpected << " zeros, got " << result.out1_samples.size();
...
const boost::ut::suite<"TxQueueSource thread safety"> thread_safety_tests = [] {
    ...
    "push from thread while draining"_test = [] {
        ...
        if (result.out0_samples.size() == totalExpected) {
            ...
            for (std::size_t i = 0; i < totalExpected; i++) {
                if (result.out0_samples[i] != expectedAll[i]) {
                    dataOk = false;
                    boost::ut::log << "first mismatch at sample " << i << ": expected " << expectedAll[i].real() << " got " << result.out0_samples[i].real();
                    break;
                }
            }
        ...
        expect(eq(result.out1_samples.size(), totalExpected)) << "out1 should have " << totalExpected << " zeros";
