...
static void write_cf32(const fs::path& path,
...
static void generate_config(const fs::path& base_dir, const Config& cfg) {
    const std::string dir_name =
        "sf" + std::to_string(cfg.sf) +
        "_cr" + std::to_string(cfg.cr) +
        "_bw" + std::to_string(cfg.bw);
    ...
    auto iq = modulate_frame(gray_mapped, cfg.sf, OS_FACTOR, SYNC_WORD,
                             PREAMBLE_LEN, zero_pad, false);
    ...
    std::fprintf(stderr, "  %s: %zu nibbles, %zu symbols, %zu IQ samples%s\n",
                 dir_name.c_str(), with_crc.size(), gray_mapped.size(),
                 iq.size(), use_ldro ? " (LDRO)" : "");
}
