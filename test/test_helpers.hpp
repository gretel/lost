...
struct VectorConfig {
    ...
    [[nodiscard]] std::string subdir() const {
        return "sf" + std::to_string(sf) +
               "_cr" + std::to_string(cr) +
               "_bw" + std::to_string(bw);
    }
};
...
inline std::vector<uint8_t> load_u8(const VectorConfig& cfg,
...
inline std::vector<uint32_t> load_u32(const VectorConfig& cfg,
                                      const std::string& filename) {
    auto path = testVectorDir() / cfg.subdir() / filename;
    std::ifstream f(path, std::ios::binary);
    if (!f) throw std::runtime_error("Cannot open test vector: " + path.string());
...
inline std::vector<std::complex<float>> load_cf32(
        const VectorConfig& cfg, const std::string& filename) {
    auto path = testVectorDir() / cfg.subdir() / filename;
    std::ifstream f(path, std::ios::binary);
    if (!f) throw std::runtime_error("Cannot open test vector: " + path.string());
...
inline std::string load_text(const VectorConfig& cfg,
                             const std::string& filename) {
    auto path = testVectorDir() / cfg.subdir() / filename;
    std::ifstream f(path);
    if (!f) throw std::runtime_error("Cannot open test vector: " + path.string());
...
inline std::vector<uint8_t> load_u8(const std::string& filename) {
    return load_u8(DEFAULT_CONFIG, filename);
}
inline std::vector<uint32_t> load_u32(const std::string& filename) {
    return load_u32(DEFAULT_CONFIG, filename);
}
inline std::vector<std::complex<float>> load_cf32(const std::string& filename) {
    return load_cf32(DEFAULT_CONFIG, filename);
}
inline std::string load_text(const std::string& filename) {
    return load_text(DEFAULT_CONFIG, filename);
}

// --- Test comparators --------------------------------------------------------

template <typename T>
void expect_vectors_equal(const std::vector<T>& actual,
                          const std::vector<T>& expected,
                          const std::string& label = "") {
    using namespace boost::ut;
    expect(actual.size() == expected.size())
        << label << " size mismatch: " << actual.size()
        << " vs " << expected.size();
